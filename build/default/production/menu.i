...
void display_list(char *[],char,char);
void view_log();
void clear_log();
void download_log();
void get_data();
...
void display_menu(unsigned char key) {
    ...
    key_level = read_switches(0);

    if(key_level == 6 && ++delay2>2000)
    {
                    delay2=0;
                    log_flag = 0;
                    list_index = 0;
    ...
    else if(log_flag == 1 || (key_level == 5 && ++delay1>2000))
    {
        ...
        if(menu_pos == 0)
        {
            ...
            {
                unsigned char key_viewlog = read_switches(0);
                clcd_print("  TIME   EV SP ",(0x80 + (0)));


                if(key_viewlog == 6 && ++delay3>2000)
                {
                ...
                if((key_viewlog == 0xFF && delay3>0) && list_event<log_count-1)
                {
                    delay3=0;
                    list_event++;

                }
                if(key == 5 && list_event >0)
                {
                    list_event--;
                }
                view_log();
    ...
    else if(key_level == 0xFF && delay1>0)
    {
        delay1 = 0;
        if(direction_flag)
        {
...
void display_list(char *menu_name[],char list_index,char direction_flag)
{
    ...
    {
        clcd_putch('*',(0x80 + (0)));
        clcd_print(menu_name[list_index],(0x80 + (1)));
        clcd_putch(' ',(0xC0 + (0)));
        clcd_print(menu_name[list_index+1],(0xC0 + (1)));
    }
    else
    {
        clcd_putch('*',(0xC0 + (0)));
...
static int list_address[10]={0x00,0x05,0x0A,0x0F,0x14,0x19,0x1E,0x23,0x28,0x2D};
...
void view_log()
{
    ...
    clcd_putch(event[temp][0],(0xC0 + (9)));
    clcd_putch(event[temp][1],(0xC0 + (10)));

    clcd_putch(' ',(0xC0 + (11)));

    temp = read_external_eeprom(list_address[list_event]+4);

    clcd_putch((temp/10)+48,(0xC0 + (12)));
    clcd_putch((temp%10)+48,(0xC0 + (13)));
...
void set_time()
{
    ...
    time_key = read_switches(0);

    if(time_key == 0xFF && (delay2 > 0) )
    {
            if(feild_flag <2)
                feild_flag++;
            else
    ...
    if(time_key == 5 && delay++ > 1000)
        {
            ...
            write_ds1307(0x02,(((time[0]-48)<<4) | ((time[1]-48) & 0x0F)));


            write_ds1307(0x01,(((time[3]-48)<<4) | ((time[4]-48) & 0x0F)));


            write_ds1307(0x00, ((((time[6]-48)<<4) | ((time[7]-48) & 0x0F)) & 0x7F));
        ...
        else if(time_key == 6 && delay2++ > 1000)
        {
            once = 1;
            TMR0IE = 0;
...
void change_password(unsigned char key)
{
    ...
    if(psw_ind >= 4)
    {
        ...
        if(psw_ind == 8)
        {
            ...
            {
                unsigned char psw_key = read_switches(0);
                static chng_delay1=0,chng_delay2=0;
                clcd_print(" CONFIRM CHANGE ",(0x80 + (0)));
                clcd_print(" Y = 5 or N = 6 ",(0xC0 + (0)));


                if(psw_key== 5 && chng_delay1++ >2000)
                {
                    write_external_eeprom(0x34,chng_psw[0]-48);
                    write_external_eeprom(0x34 +1,chng_psw[1]-48);
                    write_external_eeprom(0x34 +2,chng_psw[2]-48);
                    write_external_eeprom(0x34 +3,chng_psw[3]-48);
                    ...
                    clcd_print("CHANAGE PASSWORD",(0x80 + (0)));
                    clcd_print("   SUCCESSFUL   ",(0xC0 + (0)));
            ...
            {
                clcd_print("  ENTERED PSWD  ",(0x80 + (0)));
                clcd_print(" DOES NOT MATCH ",(0xC0 + (0)));
                psw_ind=0;
                strcpy(psw_star,"    ");
                strcpy(chng_psw_star,"    ");
                strcpy(chng_psw_temp_star,"    ");
        ...
        {
            clcd_print("  RE-ENTER PSW  ",(0x80 + (0)));
            clcd_print("            ",(0xC0 + (4)));
           ...
           if(key == 5)
            ...
            clcd_print(chng_psw_temp_star,(0xC0 + (0)));
    ...
    else
    {
        ...
        clcd_print(chng_psw_star,(0xC0 + (0)));
